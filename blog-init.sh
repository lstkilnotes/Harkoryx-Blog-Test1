...
CHECK_OUTPUT=$(npx astro check 2>&1)
...
BUILD_OUTPUT=$(npx astro build 2>&1)
BUILD_EXIT_CODE=${PIPESTATUS[0]:-$?}
if [ $BUILD_EXIT_CODE -eq 0 ] && echo "$BUILD_OUTPUT" | tail -1 | grep -qE "Complete|built in"; then
    echo "✅ 基线构建通过"
else
...
echo "  现在可以开始编辑了。"
echo "  完成后运行:"
echo "    npm run check && npm run build  # 验证"
